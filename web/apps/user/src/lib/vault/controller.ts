...
export class VaultController {
  ...
  private peerIdentities = new Map<
    string,
    { alias: string; avatar: string | null }
  >();

  private lastLocalManifest: {
    files: Array<{
      path: string;
      size: number;
      type?: string;
  ...
  private basePath(): string {
    // Keep navigation within the current i18n prefix (routes are /<lang>/...).
    const seg = window.location.pathname.split("/")[1] || "";
    const supported = SUPPORTED_LANGS as unknown as string[];
    const lang = supported.includes(seg)
      ? seg
      : (DEFAULT_LANG as unknown as string);
    return `/${lang}`;
  }
  ...
  public async init() {
    debugLog("[Vault] Controller init started", {
      projectId: redact(this.projectId),
    });
    if (!this.projectId) {
    ...
    if (unlockBtn) {
      unlockBtn.onclick = async () => {
        try {
          ...
          debugWarn("[Vault] Failed to open workspace:", e);
        }
      };
    }
  }
  ...
  private async loadProjectData(): Promise<boolean> {
    ...
    // 1. Try Load
    debugLog("[Vault] Fetching project from manager", {
      projectId: redact(this.projectId),
    });
  ...
  private renderInviteUI() {
    ...
    button.onclick = async () => {
      const link = `${window.location.origin}${this.basePath()}/vault#${this.currentProject!.id}.${this.currentProject!.projectMasterKey}`;
      await navigator.clipboard.writeText(link);
      const prev = button.textContent;
      button.textContent = "Copied!";
      setTimeout(
        () => (button.textContent = prev || "Copy Instant Link"),
        2000
      );
    };
  }
  ...
  private async broadcastIdentity() {
    ...
    if (identity) {
      ...
      this.vaultManager.sendIdentity(
  ...
  private async initVaultConnection(projectId: string, secretKey: string) {
    ...
    this.vaultManager.setListeners({
      onStateChange: (state) => {
        ...
        if (dot)
          dot.className = `w-2.5 h-2.5 rounded-full ${state === "connected" ? "bg-green-500" : "bg-yellow-500 animate-pulse"}`;
        if (status)
          status.textContent =
            state === "connected" ? "P2P Connected" : "Waiting for peers...";
        ...
        if (state === "connected") {
          ...
          if (
            this.ephemeralMode ||
            this.currentProject?.name === "Loading Project..."
          ) {
      ...
      onIdentity: (identity, peerId) => {
        ...
        this.appendChatMessage(`Connected as ${identity.alias}`, "system");
      },
      onMessage: (msg) => {
      ...
      onFileReceived: async (fileId, meta, blob) => {
        ...
        const name =
          typeof safeMeta.name === "string" && safeMeta.name
            ? safeMeta.name
            : `file-${fileId}`;
        const size =
          typeof safeMeta.size === "number"
            ? safeMeta.size
        ...
        try {
          if (this.projectId) {
            ...
            const rootFolder = storedPath.startsWith("project/")
              ? "project"
              : "chat";
            ...
            const html = `
                                                ...
                                                `.trim();
            this.appendChatMessage(html, "system", undefined, true);
            return;
          }
        } catch (e) {
        ...
        this.appendChatMessage(link, "system", undefined, true);
      ...
      onMetadata: async (meta) => {
        debugLog("[Vault] Metadata Received", {
          hasName: Boolean((meta as any)?.name),
        });
        const nameEl = document.getElementById("project-name");
        ...
        if (this.ephemeralMode || this.currentProject?.name !== meta.name) {
          ...
          this.appendChatMessage(
            `Project identified as ${meta.name}`,
            "system"
          );
        }
      },
      ...
      onManifest: (manifest, peerId) => {
        try {
          const files =
            ...
            Array.isArray((manifest as any).files)
          ...
          this.peerManifests.set(peerId, {
            files,
            generatedAt:
              typeof (manifest as any)?.generatedAt === "number"
                ? (manifest as any).generatedAt
                : Date.now(),
          });
  ...
  private async publishManifest(targetPeerId?: string) {
    ...
    this.vaultManager.sendManifest(
      { projectId: this.projectId, ...manifest },
      targetPeerId
    );
  }

  private async computeMissingPlan(): Promise<{
    requestsByPeer: Map<string, string[]>;
    missingCount: number;
  }> {
    ...
    for (const [path, peerId] of availability.entries()) {
      ...
      const peerSize = this.peerManifests
        .get(peerId)
        ?.files.find((x) => x.path === path)?.size;
      const isMissing = localSize == null;
      const sizeMismatch =
        localSize != null && peerSize != null && localSize !== peerSize;
  ...
  private async syncMissingProjectFiles() {
    ...
    this.appendChatMessage(
      `Requested ${requested} file(s) from peers…`,
      "system"
    );
  }
  ...
  private async handleFileRequest(paths: string[], peerId: string) {
    ...
    if (!getActiveHandle()) {
      this.appendChatMessage(
        "Cannot serve requested files: no Vault folder open.",
        "system"
      );
      return;
    }
  ...
  private bindUI() {
    ...
    document.addEventListener(
      ...
      (ev) => {
        ...
        if (action === "preview-file") {
          const pid =
            target.getAttribute("data-project-id") || this.projectId || "";
    ...
    (window as any).holiUploadProjectFiles = async (
      files: File[],
      folder: string,
      announce: boolean
    ) => {
      try {
        ...
        if ((e as any)?.message === "No active workspace") {
          this.appendChatMessage(
            "Open your Vault folder to upload project files.",
            "system"
          );
        }
        throw e;
  ...
  public async uploadProjectFiles(
    files: File[],
    subfolder: string,
    announceToChat: boolean
  ) {
    if (!this.vaultManager || !this.projectId)
