...
export interface VaultEvents {
  ...
  onIdentity?: (
    identity: { alias: string; avatar: string | null },
    peerId: string
  ) => void;
...
export class TrysteroVaultManager {
  ...
  public async join(secretKey: string) {
    ...
    this.room = joinTrysteroRoom(
      { appId: "holi-vault", password: secretKey },
      derivedRoomId
    );
    ...
    getFile((a: any, b: any, c: any) => {
      ...
      if (blobArg) {
        ...
        const ab = args.find((x) => x instanceof ArrayBuffer) as
          | ArrayBuffer
          | undefined;
        const u8 = args.find((x) => x instanceof Uint8Array) as
          | Uint8Array
          | undefined;
    ...
    getReqFiles((data: any, peerId: string) => {
      const paths =
        data && typeof data === "object" && Array.isArray((data as any).paths)
  ...
  public sendFile(
    file: File,
    metaNameOverride?: string,
    targetPeerId?: string
  ) {
    if (!this.sendFileAction) return;
    const metaName =
      typeof metaNameOverride === "string" && metaNameOverride.trim()
        ? metaNameOverride.trim()
        : file.name;

    this.sendFileAction(file, targetPeerId ?? null, {
      name: metaName,
      type: file.type,
      size: file.size,
    });
  ...
  public leave() {
    if (this.room) {
      ...
      this.room = null;
      this.peers.clear();
      this.sendChatAction = null;
      this.sendFileAction = null;
      this.sendMetadataAction = null;
      this.requestMetadataAction = null;
      this.sendIdentityAction = null;
      this.sendSyncAction = null;
      this.sendManifestAction = null;
      this.requestFilesAction = null;
