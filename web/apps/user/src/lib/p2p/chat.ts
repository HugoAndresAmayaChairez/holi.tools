...
export class ChatManager {
  ...
  constructor(channel: RTCDataChannel, options?: ChatManagerOptions) {
  ...
  close() {
    this.stopHeartbeat();
    this.failPendingOutgoingFiles(new Error("Connection closed"));
    try {
  ...
  private reportDisconnected() {
    if (this.reportedDisconnect) return;
    this.reportedDisconnect = true;
    this.stopHeartbeat();
    this.failPendingOutgoingFiles(new Error("Connection lost"));
    this.emit({ type: "disconnected" });
  }

  private failPendingOutgoingFiles(error: Error) {
    for (const pending of this.pendingOutgoingFileAccept.values()) {
      pending.reject(error);
    }
    this.pendingOutgoingFileAccept.clear();
  }
  ...
  /**
   * The signaling adapter can preserve a room while a browser refresh replaces
   * its peer connection. Reset the liveness window before heartbeat resumes so
   * the replacement peer gets a full interval in which to answer.
   */
  ...
  private ensureWasmReady() {
    if (!this.wasmReady) {
      this.wasmReady = initWasmP2p().then(() => undefined);
    }
      ...
      this.receiveQueue = this.receiveQueue
        .then(async () => {
          ...
          if (event.data instanceof ArrayBuffer) {
            ...
            console.warn(
              "[Chat] Unknown message data type:",
              typeof event.data
            );
            return;
          }
          ...
          try {
            ...
            if (!this.sessionKeyBytes) {
              ...
              if (bytes.length < 2 || bytes[0] !== 72 || bytes[1] !== 79) {
                // 'H', 'O'
            ...
            if (
              decodedBytes.length === 9 &&
              (decodedBytes[0] === HEARTBEAT_PING ||
                decodedBytes[0] === HEARTBEAT_PONG)
            ) {
              await this.handleHeartbeat(decodedBytes);
              return;
            }
            ...
            if (frameType === 0x20) {
              ...
              if (this.options.onIncomingFileOffer) {
                decision =
                  await this.options.onIncomingFileOffer(incomingOffer);
              } else {
                decision =
              ...
              if (rejectReason) {
                const rejectBytes = encode_file_reject_v1(
                  incomingOffer.id,
                  rejectReason
                );
            ...
            if (frameType === 0x23) {
              ...
              const chunkCopy = new Uint8Array(data.byteLength);
              chunkCopy.set(data);
              const chunkBuf = chunkCopy.buffer;
        ...
        .catch((error) => {
          console.error("[Chat] Failed to process incoming frame:", error);
        });
    };
  ...
  private async handleIncoming(msg: ChatMessage) {
    switch (msg.type) {
      ...
      case "file-start":
        ...
        this.emit({
          ...
          message: {
            ...msg,
            type: "file-start",
            content: `Receiving ${msg.filename}...`,
          } as any,
        });
        break;
      ...
      case "file-chunk": {
        ...
        if (!fileCtx) {
          debugWarn("[Chat] Chunk for unknown file", {
            fileId: redact(msg.id),
          });
          return;
        }
        ...
        if (msg.chunkIndex % 25 === 0) {
          debugLog("[Chat] Receiving progress", {
      ...
      case "file-end": {
        ...
        if (completedFile) {
          // Reassemble Blob
          const blob = new Blob(completedFile.chunks, {
            type: completedFile.metadata.mimeType,
          });
          ...
          debugWarn("[Chat] file-end with no context", {
            fileId: redact(msg.id),
          });
        }
        break;
  ...
  async sendText(content: string) {
    ...
    try {
      ...
      await this.sendFrame(new Uint8Array(frameBytes));
    } catch (e) {
      console.error("[Chat] Failed to send text frame:", e);
      if (this.sessionKeyBytes) {
        this.emit({
          type: "encryption_error",
          message:
            "Failed to send message. Check that both sides are still connected.",
        });
      } else {
        this.reportDisconnected();
      }
  ...
  private async waitForBufferedAmountBelow(targetBytes: number) {
    ...
    await new Promise<void>((resolve) => {
      ...
      (this.channel as any).addEventListener("bufferedamountlow", handler, {
        once: true,
      });
    });
  }
  ...
  async sendFile(file: File) {
    ...
    const offerBytes = encode_file_offer_v1(
      id,
      file.name,
      file.type || "application/octet-stream",
      BigInt(file.size)
    );
    // Register the waiter before sending the offer. A fast local peer can reply
    // in the same event-loop turn, and that acceptance must not be missed.
    ...
    try {
      ...
      // The receiver may need time to review the in-chat confirmation.
    ...
    while (offset < buffer.byteLength) {
      ...
      const chunkBytes = encode_file_chunk_v1(
        id,
        chunkIndex++,
      ...
      if (chunkIndex % 25 === 0)
        debugLog("[Chat] Sent chunks", { sent: chunkIndex });
