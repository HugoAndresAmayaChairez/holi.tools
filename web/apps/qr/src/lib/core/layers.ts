...
export function ensureLayersConfig(config: any): QRLayersConfig {
    ...
    layers.bg.boundsScale = Math.max(1, layers.bg.boundsScale ?? defaults.bg.boundsScale);
    layers.bg.scale = Math.max(0.01, layers.bg.scale ?? defaults.bg.scale);
    layers.paper.opacity = clamp01(layers.paper.opacity ?? defaults.paper.opacity);
    layers.paper.boundsScale = Math.max(1, layers.paper.boundsScale ?? defaults.paper.boundsScale);
    layers.paper.scale = Math.max(0.01, layers.paper.scale ?? defaults.paper.scale);
