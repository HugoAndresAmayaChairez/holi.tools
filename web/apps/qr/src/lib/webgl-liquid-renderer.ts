...
export class WebGLLiquidRenderer {
    ...
    async init(): Promise<boolean> {
        ...
        if (!this.blurProgram || !this.gooeyProgram || !this.shapeProgram || !this.maskProgram || !this.logoProgram || !this.compositeProgram) {
    ...
    private renderLogo(config: RenderConfig) {
        ...
        gl.uniform1f(gl.getUniformLocation(this.logoProgram, 'uOpacity'), Math.max(0.0, Math.min(1.0, config.logoOpacity ?? 1.0)));
    ...
    render(config: RenderConfig) {
        ...
        const dbgView = readValue<'mask' | 'ink' | 'under' | 'final'>('__qrDebugView', '_qrDebugView');
        const dbgMode = dbgView === 'mask' ? 1 : dbgView === 'ink' ? 2 : dbgView === 'under' ? 3 : 0;
        const forceCompositeForDebug = dbgMode !== 0;

        const dbgReadback = readBool('__qrDebugReadback', '_qrDebugReadback');

        const dbg = readBool('__qrDebugLayers', '_qrDebugLayers', '__qrDebugLaters', '_qrDebugLaters');
        if (dbg) {
        ...
        const needsComposite = forceCompositeForDebug || hasBase || hasArt || hasComplement || hasPaperTexture || hasInkTexture || hasBlur;
        ...
        gl.uniform1i(gl.getUniformLocation(this.shapeProgram!, 'uBodyMaskAtlas'), 1);
        gl.activeTexture(gl.TEXTURE2);
        gl.bindTexture(gl.TEXTURE_2D, ts.eyeMaskTexture);
        gl.uniform1i(gl.getUniformLocation(this.shapeProgram!, 'uEyeMask'), 2);
        gl.uniform4fv(gl.getUniformLocation(this.shapeProgram!, 'uColor'), config.color);
        gl.uniform1i(gl.getUniformLocation(this.shapeProgram!, 'uQRSize'), config.qrSize || this.matrixSize);

        // Gradient & Noise Uniforms
        gl.uniform4fv(gl.getUniformLocation(this.shapeProgram!, 'uColor2'), config.gradientColor2 || [0, 0, 0, 1]);
        ...
        if (needsMask) {
            ...
            gl.uniform1i(gl.getUniformLocation(this.maskProgram!, 'uQRSize'), config.qrSize || this.matrixSize);
            ...
            if (dbgReadback) {
                ...
                const totalModules = (config.qrSize || this.matrixSize) + (paddingModules * 2.0);
        ...
        if (hasBlur) {
            ...
            gl.uniform4fv(gl.getUniformLocation(this.gooeyProgram!, 'uColor'), config.color);
            gl.uniform4fv(gl.getUniformLocation(this.gooeyProgram!, 'uColor2'), config.gradientColor2 || [0, 0, 0, 1]);
            ...
            if (hasComplement) {
                ...
                gl.uniform1i(gl.getUniformLocation(this.gooeyProgram!, 'uTexture'), 0);
                gl.uniform1f(gl.getUniformLocation(this.gooeyProgram!, 'uThreshold'), config.threshold);
                gl.uniform4fv(gl.getUniformLocation(this.gooeyProgram!, 'uColor'), [1, 1, 1, 1]);
                gl.uniform4fv(gl.getUniformLocation(this.gooeyProgram!, 'uColor2'), [1, 1, 1, 1]);
                gl.uniform1i(gl.getUniformLocation(this.gooeyProgram!, 'uGradientType'), 0);
                gl.uniform1f(gl.getUniformLocation(this.gooeyProgram!, 'uGradientAngle'), 0.0);
                gl.uniform1f(gl.getUniformLocation(this.gooeyProgram!, 'uNoiseAmount'), 0.0);
                gl.uniform1f(gl.getUniformLocation(this.gooeyProgram!, 'uNoiseScale'), 0.0);
        ...
        if (needsComposite) {
            ...
            gl.bindTexture(gl.TEXTURE_2D,
                needsMask
                    ? (hasBlur ? this.maskPongTexture : this.maskPingTexture)
                    : this.blankTexture
            );
            gl.uniform1i(gl.getUniformLocation(this.compositeProgram!, 'uMaskTexture'), 3);
            ...
            gl.activeTexture(gl.TEXTURE2);
            gl.bindTexture(gl.TEXTURE_2D, hasPaperTexture ? ts.paperTexture : this.blankTexture);
            gl.uniform1i(gl.getUniformLocation(this.compositeProgram!, 'uPaperTexture'), 2);

            gl.activeTexture(gl.TEXTURE4);
            gl.bindTexture(gl.TEXTURE_2D, hasInkTexture ? ts.inkTexture : this.blankTexture);
            gl.uniform1i(gl.getUniformLocation(this.compositeProgram!, 'uInkTexture'), 4);

            // BG base color
            gl.uniform4fv(gl.getUniformLocation(this.compositeProgram!, 'uBaseColor'), (config.bgBaseColor || [0, 0, 0, 0]) as [number, number, number, number]);

            // Paper/complement color
            const paper = (config.qrBgColor || [0, 0, 0, 0]) as [number, number, number, number];
            gl.uniform4fv(gl.getUniformLocation(this.compositeProgram!, 'uPaperColor'), paper);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uPaperTexEnabled'), hasPaperTexture ? 1.0 : 0.0);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uPaperAspect'), ts.paperAspect);
            gl.uniform1i(gl.getUniformLocation(this.compositeProgram!, 'uPaperFitMode'), 0);
            const paperRotRad = (((config.paperRotation || 0) * Math.PI) / 180);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uPaperRotation'), paperRotRad);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uPaperScale'), config.paperScale || 1.0);
            gl.uniform2f(gl.getUniformLocation(this.compositeProgram!, 'uPaperOffset'), config.paperOffsetX || 0, config.paperOffsetY || 0);

            // Ink texture override
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uInkTexEnabled'), hasInkTexture ? 1.0 : 0.0);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uInkAspect'), ts.inkAspect);
            gl.uniform1i(gl.getUniformLocation(this.compositeProgram!, 'uInkFitMode'), 0);
            const inkRotRad = (((config.inkRotation || 0) * Math.PI) / 180);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uInkRotation'), inkRotRad);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uInkScale'), config.inkScale || 1.0);
            gl.uniform2f(gl.getUniformLocation(this.compositeProgram!, 'uInkOffset'), config.inkOffsetX || 0, config.inkOffsetY || 0);

            // Ink enabled
            const inkEnabled = (config.inkEnabled ?? ((config.color?.[3] ?? 1) > 0.001));
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uInkEnabled'), inkEnabled ? 1.0 : 0.0);

            gl.uniform1i(gl.getUniformLocation(this.compositeProgram!, 'uDebugMode'), dbgMode);

            // Background (art) uniforms
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uOpacity'), hasArt ? (config.artOpacity ?? 1.0) : 0.0);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uArtAspect'), ts.artAspect);
            gl.uniform1f(gl.getUniformLocation(this.compositeProgram!, 'uContainerAspect'), 1.0);
            ...
            gl.uniform1i(gl.getUniformLocation(this.compositeProgram!, 'uPaperFitMode'), paperFitMode);

            // Map ink fit mode
            let inkFitMode = 0;
            if (config.inkFit === 'contain') inkFitMode = 1;
            else if (config.inkFit === 'fill') inkFitMode = 2;
            gl.uniform1i(gl.getUniformLocation(this.compositeProgram!, 'uInkFitMode'), inkFitMode);
    ...
    async captureHighRes(config: RenderConfig, targetSize: number = 2048): Promise<Blob | null> {
        ...
        const safeTarget = (Number.isFinite(targetSize) && targetSize > 0)
