...
import { WasmSvgRenderer, type RenderConfig as SvgRenderConfig } from './wasm-svg-renderer';
...
export class QRController {
    ...
    public getFullRenderConfig(): RenderConfig {
        ...
        const rawBgBase = layers.bg.enabled
            ? (hexToRgba(layers.bg.color || '#ffffff') as [number, number, number, number])
        ...
        const normalizeGradientAngle = (raw: unknown): number => {
            const angle = typeof raw === 'number' ? raw : (typeof raw === 'string' ? parseFloat(raw) : 0);
        ...
        const wantsTransparentInk = !layers.ink.enabled || (layers.ink.opacity ?? 1) <= 0.001 || inkAlpha <= 0.001;

        const gradientColor2Rgba = hexToRgba(layers.ink.gradient.color2 || safeFg || '#000000');
        ...
        return {
            ...
            color: fgRgba,
            inkEnabled: layers.ink.enabled && (layers.ink.opacity ?? 1) > 0.001 && (fgRgba[3] ?? 1) > 0.001,
            ...
            backgroundColor: parseBgColor(layers.paper.enabled ? paperColor : 'transparent') ?? [1, 1, 1],
            bgBaseColor,
            ...
            logoBgColor: state.config.logoBgColor ? hexToRgb(state.config.logoBgColor).slice(0, 3) as [number, number, number] : [1, 1, 1],
    ...
    private updateQrFrameBackground() {
        ...
        const layers = this.getLayers();
        const bgAlpha = layers.paper.enabled ? ((hexToRgba(layers.paper.color || '#ffffff')[3] ?? 1) * (layers.paper.opacity ?? 1)) : 0;
        const fgAlpha = layers.ink.enabled ? ((hexToRgba(layers.ink.color || '#000000')[3] ?? 1) * (layers.ink.opacity ?? 1)) : 0;
        const wantsTransparentBg = !layers.paper.enabled || bgAlpha <= 0.001;
        const wantsTransparentInk = !layers.ink.enabled || fgAlpha <= 0.001;

        this.els.qrFrame.classList.toggle('transparent-bg', (wantsTransparentBg || wantsTransparentInk));
        this.els.qrFrame.style.backgroundColor = '';
    }
    ...
    private getSvgRenderConfig(): SvgRenderConfig {
        ...
        let svgGradientType: SvgRenderConfig['gradientType'] | undefined = undefined;
        if (numericType === 1) svgGradientType = 'linear';
        else if (numericType === 2) svgGradientType = 'radial';

        const gradientColor2 = layers.ink.gradient.color2 || layers.ink.color || '#000000';
        const rawGradientAngle = layers.ink.gradient.angle || 0;
        const gradientAngle = (Math.abs(rawGradientAngle) > (2 * Math.PI + 0.001) && Math.abs(rawGradientAngle) <= 360)
        ...
        const svgBgColor = (layers.paper.enabled && paperAlpha > 0.001) ? toCssRgba(paperColor, paperOpacity) : 'transparent';
        const svgFgColor = (layers.ink.enabled && inkOpacity > 0.001) ? toCssRgba(layers.ink.color || '#000000', inkOpacity) : 'transparent';
        const bgLayerOpacity = layers.bg.enabled ? (layers.bg.opacity ?? 1) : 0;
        ...
        return {
            ...
            gradientColors: [toCssRgba(layers.ink.color || '#000000', inkOpacity), toCssRgba(gradientColor2, inkOpacity)],
    ...
    public async renderQR() {
        ...
        if (this.webglRenderer) {
            const layers = this.getLayers();
            const artUrl = (layers.bg.enabled && layers.bg.image) ? layers.bg.image : null;
            const logoUrl = (layers.logo.enabled ? (layers.logo.image ?? null) : null);
            const paperUrl = (layers.paper.enabled && layers.paper.image) ? layers.paper.image : null;
            const inkUrl = (layers.ink.enabled && layers.ink.image) ? layers.ink.image : null;
    ...
    public async copyQRToClipboard() {
        ...
        try {
            setBusy(true);
            let blob = await this.getHighResSnapshot(2048);
            if (!blob) {
    ...
    async getHighResSnapshot(targetSize: number = 2048): Promise<Blob | null> {
        ...
        if (config.paperImage) {
            await this.webglRenderer.setPaperImage(config.paperImage);
        }
        if (config.inkImage) {
            await this.webglRenderer.setInkImage(config.inkImage);
        }
