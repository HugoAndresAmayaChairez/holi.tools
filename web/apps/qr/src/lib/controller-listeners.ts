 ...
 */
export function attachClickDelegation(
  ctrl: QRController,
  signal: AbortSignal
): void {
...
export function attachInputListeners(
  ...
  document.body.addEventListener(
    ...
    (e) => {
      if (
        (e.target as HTMLElement).classList.contains("input-field") &&
 ...
 */
export function attachTypeChangeListener(
  ctrl: QRController,
  signal: AbortSignal
): void {
  window.addEventListener(
    "qr-type-changed",
    (async (e: CustomEvent<{ type?: string }>) => {
      const type = e.detail?.type || (window as any).currentContentType;
      if (type === "scan") {
        qrScanner.activate();
      } else {
        await ctrl.injectBrandLogo(type);
        ctrl.debouncedAutoGenerate();
      }
    }) as EventListener,
    { signal }
...
export function attachEffectListeners(
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      if (
        ctrl.webglRenderer &&
        (layers.ink.liquid.enabled || layers.bg.enabled)
      ) {
 ...
 */
export function attachLogoListeners(
  ctrl: QRController,
  signal: AbortSignal
): void {
  const logoInput = document.getElementById("logo-upload") as HTMLInputElement;
  if (logoInput)
    logoInput.addEventListener("change", (e) => ctrl.handleLogoUpload(e), {
      signal,
    });
