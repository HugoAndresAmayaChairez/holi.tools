...
/**
 * Convert hex color to normalized RGBA tuple (0-1 range)
 * @param hex - Hex color string (e.g., '#ff0000' or 'ff0000') or 'transparent'
 * @returns RGBA tuple with values 0-1
 */
export function hexToRgba(hex: string): [number, number, number, number] {
  ...
  if (isNaN(bigint)) return [0, 0, 0, 1];

  // Check if alpha is included in hex (e.g. #RRGGBBAA)
